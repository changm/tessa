...
testcases[index++] = new TestCase ( SECTION, "'Sally and Fred are sure to come'.match(/^[a-z\\s]*/i)",
...
testcases[index++] = new TestCase ( SECTION, "'test123W+xyz'.match(new RegExp('^[a-z]*[0-9]+[A-Z]?.(123|xyz)$'))",
...
testcases[index++] = new TestCase ( SECTION, "'number one 12365 number two 9898'.match(/(\d+)\D+(\d+)/)",
...
testcases[index++] = new TestCase ( SECTION, "'See Spot run.'.match(simpleSentence)",
...
testcases[index++] = new TestCase ( SECTION, "'I like it. What's up? I said NO!'.match(simpleSentence)",
...
testcases[index++] = new TestCase ( SECTION, "'the quick brown fox jumped over the lazy dogs'.match(/((\\w+)\\s*)+/)",
