...
echo "running android shell remotely on host: $SHELLSERVER port: $SHELLPORT"
test "$SHELLSERVER" = "" && {
...
echo "acquiring lock"
$workdir/socketserver-client.py lock get
res=$?
test "$res" = "0" || {
    echo "message: acquiring lock failed"
    exit 1
}

echo "running setup $branch/${change}-${changeid}"
$workdir/socketserver-client.py setup_android $branch/${change}-${changeid}
res=$?
test "$res" = "0" || {
    echo "message: setup failed"
    ./shell-client-setup.py lock release
    exit 1
}
