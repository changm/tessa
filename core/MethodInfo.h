...
namespace avmplus
{
    ...
    public:
		...
		FprMethodProc implTessa() const;
        bool isInterpreted() const;
        Atom invoke(MethodEnv*, int32_t, Atom*);

		int testThisCall(MethodEnv*, int32_t);
    };
...
// end AVMPLUS_UNCHECKED_HACK
	...
	private:
		void doTessaOptimizations(ASFunction* tessaMethod, AvmCore* core, Verifier* verifier); 
		void compileWithTessa(Verifier* verifier);
		void generateMachineCode(ASFunction* tessaMethod, AvmCore* core);
...
#ifdef INTEL_VTUNE
		void setNativeImpl(GprMethodProc p, int32_t codeSize);
#endif

		void setTessaFunction(ASFunction* p);
		void setTessaInterpImpl();
		void setTessaNativeImpl(ASFunction* function, void* nativeCodeLocation, bool setInvoker = false);
		ASFunction* getTessaFunction();
