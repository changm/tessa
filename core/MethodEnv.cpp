...
namespace avmplus
{
...
#ifdef DEBUG
	void printMethodEnvName(MethodInfo* methodInfo) {
		int methodIid = methodInfo->iid();
    ...
    {
		//printMethodEnvName(this->method);
	...
	Atom MethodEnv::coerceEnter_Tessa_interp(MethodEnv* env, int32_t argc, Atom* argv) {
		MethodInfo* methodInfo = env->method;
		MethodSignaturep methodSignature = env->get_ms();

		AvmCore* core = env->core();
		TessaVM::ASFunction* function = methodInfo->getTessaFunction();

		if (function == NULL) {
			return MethodInfo::verifyCoerceEnter(env, argc, argv);
		} else {
			TessaAssert(core->config.tessaInterp);
			TessaInterpreter* interpreter = new (core->gc) TessaInterpreter(core, methodInfo->pool());
			return interpreter->interpret(env, argc, argv, function);
		}
	}
	 ...
	 */
	Atom MethodEnv::coerceEnter_Tessa_native(MethodEnv* env, int32_t argc, Atom* argv) {
		TessaAssert(false);	// Should be using MethodInfo::implgpr
		AvmCore* core = env->core();
		TessaAssert(!core->config.tessaInterp);
		MethodInfo* methodInfo = env->method;
		TessaVM::ASFunction* function = methodInfo->getTessaFunction();
		...
		if (function == NULL) {
			...
			void* nativePointer = function->getNativeCodeLocation();
			TessaAssert(nativePointer != NULL);
			// So we don't come here on the next jit call
			methodInfo->setTessaNativeImpl(function, nativePointer, true);

			Atom (*functionPointer)(MethodEnv*, int32_t, Atom*) = (Atom(*)(MethodEnv*, int32_t, Atom*))nativePointer;
			return functionPointer(env, argc, argv);
		}
	}
