...
namespace avmshell
{
    ...
    ShellCoreSettings::ShellCoreSettings()
		...
		, useTessa(AvmCore::useTessa_default)
		, tessaInline(AvmCore::tessaInline_default)
		, tessaTypePropagation(AvmCore::tessaTypePropagation_default)
		, tessaVerbose(AvmCore::tessaVerbose_default)
		, tessaInterp(AvmCore::tessaInterp_default)
    ...
    bool ShellCore::setup(ShellCoreSettings& settings)
    {
		...
		config.useTessa = settings.useTessa;
		config.tessaVerbose = settings.tessaVerbose;
		config.tessaInterp = settings.tessaInterp;
		config.tessaInline = settings.tessaInline;
		config.tessaTypePropagation = settings.tessaTypePropagation;
        ...
        TRY(this, kCatchAction_ReportAsError)
        {
			...
			this->getActiveCore()->setIsUserScript(false);
...
#endif
			this->getActiveCore()->setIsUserScript(true);
            return true;
        }
        CATCH(Exception *exception)
        {
