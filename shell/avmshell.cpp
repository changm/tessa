        ...
        for (int i=1; i < argc ; i++) {
            ...
            if (arg[0] == '-')
            {
                ...
                if (arg[1] == 'D') {
					...
					else if(!VMPI_strcmp(arg+2, "tessa")) {
						settings.useTessa = true;
					}
					else if(!VMPI_strcmp(arg+2, "tessa_verbose")) {
						settings.tessaVerbose = true;
					}
					else if (!VMPI_strcmp(arg+2, "tessa_inline")) {
						settings.tessaInline = true;
					}
					else if(!VMPI_strcmp(arg+2, "tessa_interp")) {
						settings.tessaInterp = true;
					} else if (!VMPI_strcmp(arg + 2, "tessa_tp")) {
						settings.tessaTypePropagation = true;
					} else {
...
#endif
		AvmLog("          [-Dtessa]     Use Tessa Intermediate Representation\n");
		AvmLog("          [-Dtessa_verbose] Verbose Tessa Output\n");
        AvmLog("          [-Dtessa_interp] Use the Tessa Interpreter\n");
        AvmLog("          [-Dtessa_inline] Enable TESSA Inlining\n");
        AvmLog("          [-Dtessa_tp] Enable TESSA Type Propagation\n");
#ifdef FEATURE_NANOJIT
